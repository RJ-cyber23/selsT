...
<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4 gap-2">
    <div>
        <h2 class="fw-bold mb-0 text-white">Categories</h2>
        <p class="text-white-50 mb-0">{{ count($categories) }} categor(ies)</p>
    </div>
    <div class="d-flex gap-2 align-items-center flex-wrap">
        <form method="GET" class="d-flex align-items-center gap-2">
            <input type="text" name="search" class="form-control form-control-sm" placeholder="Search categories..." value="{{ $search ?? '' }}" style="min-width:0">
            @if($search)
                <a href="/categories" class="btn btn-outline-secondary btn-sm flex-shrink-0">Clear</a>
            @endif
        ...
        <div class="card border-0 shadow-sm h-100">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start mb-3">
                    <h6 class="card-title fw-semibold mb-0">{{ $cat->category_name }}</h6>
                    <span class="badge bg-secondary">#{{ $cat->category_id }}</span>
                </div>
                <div class="d-flex gap-2">
                    <button class="btn btn-outline-primary btn-sm flex-grow-1"
                        onclick="openEditModal(this)"
                        data-id="{{ $cat->category_id }}"
                        data-name="{{ $cat->category_name }}">Edit</button>
                    <button class="btn btn-outline-danger btn-sm flex-grow-1"
                        onclick="openDeleteModal(this)"
                        data-id="{{ $cat->category_id }}"
                        data-name="{{ $cat->category_name }}">Delete</button>
                </div>
            </div>
        </div>
    </div>
...
<div class="text-center py-5">
    ...
    <h5 class="text-white mt-2">No categories yet</h5>
    <p class="text-white-50">Start by adding a new category.</p>
...
<div class="modal fade" id="deleteModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title fw-semibold">Delete Category</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
...
function openEditModal(btn) {
    ...
    f.action = '/categories/' + btn.dataset.id;
    f.querySelector('[name="category_name"]').value = btn.dataset.name;
    new bootstrap.Modal(document.getElementById('editModal')).show();
}
function openDeleteModal(btn) {
    document.getElementById('deleteForm').action = '/categories/' + btn.dataset.id;
...
document.addEventListener('DOMContentLoaded', function() {
    var m = '{{ old("_modal") }}';
    if (m === 'edit') {
        document.getElementById('editForm').action = '/categories/' + ('{{ old("_id") }}');
        new bootstrap.Modal(document.getElementById('editModal')).show();
    } else if (m === 'add') {
        new bootstrap.Modal(document.getElementById('addModal')).show();
    }
